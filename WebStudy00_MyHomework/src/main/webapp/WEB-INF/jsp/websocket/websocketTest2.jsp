...
<div id="container">
	...
	<main>
		...
		<footer>
			<textarea placeholder="Type your message" id="msgContent"></textarea>
			<img
				src="https://s3-us-west-2.amazonaws.com/s.cdpn.io/1940306/ico_picture.png"
				alt=""> <img
				src="https://s3-us-west-2.amazonaws.com/s.cdpn.io/1940306/ico_file.png"
				alt=""> <input type="button" value="Send" id="sendBtn" />
		</footer>
	</main>
</div>
<div>
	<button id="openSocketBtn">Open</button>
	<button id="closeSocketBtn">close</button>
</div>
<script>
		$(function () {
			var ws;
			...
			$("#openSocketBtn").on("click", function () {
				...
				//웹소켓 객체 만드는 코드
				ws = new WebSocket(
					"ws://192.168.35.124/WebStudy00_MyHomework/basic/chat");

				//웹소켓이 성공적으로 열리면 이 함수 실행
			...
			$("#sendBtn").on("click", function () {
				...
				let sender = "박민주";  //나중에 세션id 가져오기
				let chatRoomCd = "20231102C0001";
				...
				localStorage.setItem(`chatting\${num}`, JSON.stringify(chatting));

				let getChatting = localStorage.getItem(`chatting\${num}`);
				console.log("가져온거 : ", JSON.parse(getChatting));
			...
			function writeResponse(getChatting) {
				console.log("위 메소드 :  ", getChatting);
				let tag = makeChatBaloon(getChatting);
				$('#chat>li:last-child').after(tag);
			}
			
			function makeChatBaloon(getChatting){
				console.log("아래 메소드 :  ", getChatting);
				
				var data = JSON.parse(getChatting);
				...
				let tag = `
					...
					<div class="triangle"></div>
					<div class="message">\${data.msgContent}</div>
				</li>
				`;
				return tag;
			}
